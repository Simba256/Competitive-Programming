...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        for(int j=0;j<m;j++){
            if(v[i][j]=='#'){
                f = 1;
                nv.pb(mp(i+empi*999999,j+empj[j]*999999));
            }
        }if(!f) empi++;
    }n = nv.size();
    cout<<n;nl;
    ...
    for(int i=0;i<n;i++){
        for(int j=i+1;j<n;j++){
            if((i==0&&j==6)||(i==2&&j==5)||(i==7&&j==8)) cout<<abs(nv[i].first-nv[j].first) + abs(nv[i].second-nv[j].second)<<'\n';
            ans += abs(nv[i].first-nv[j].first) + abs(nv[i].second-nv[j].second);
        }
    }cout<<ans;
