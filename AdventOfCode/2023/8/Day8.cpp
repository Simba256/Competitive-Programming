...
ll findcount(string &s,map<string,pair<string,string>> &m, string &path){
    ll coun = 0, n = path.length();
    string curr = s;
    while(1){
        for(int i=0;i<n;i++){
            if(path[i]=='L') curr = m[curr].first;
            else curr = m[curr].second;
            coun++;
            if(curr[2]=='Z') return coun;
        }
    }
}
...
void solve(){
    string lr,c = "AAA",s,t,u;
    cin>>lr;
    ve<string> v;
    map<string,pair<string,string>> m;
    for(int i=0;i<750;i++){
        cin>>s>>t>>u;
        if(s[2]=='A') v.push_back(s);
        m[s] = {t,u};
    }ll n = lr.length();
    ll count = 0;
    vll ans;
    for(auto x:v){
        ans.pb(findcount(x,m,lr));
    }ll finalans = 1;
    for(auto x:ans){
        finalans = lcm(finalans,x);
    }cout<<finalans;

    // while(1){
    //     for(int i=0;i<n;i++){
    //         if(lr[i]=='L') c = m[c].first;
    //         else c = m[c].second;
    //         count++;
    //         if(c=="ZZZ"){
    //             cout<<count;
