...
void solve(){
    // n - max(movies running at a time) - 1
    ...
    cin>>n;
    vp a(n);
    rep(0,n,i) cin>>a[i].first>>a[i].second;
    so(a);
    ll curr = a[0].first, ans = 1, people = 1;
    for(int i=1,j=0;j<n;){
        // cout<<"curr:"<<curr<<" ans:"<<ans<<" p:"<<people;nl;
        if(i<n) curr = min(a[i].first,a[j].second);
        else curr = a[j].second;
        if(i<n && curr==a[i].first){
            i++;
            people++;
        }ans = max(ans,people);
        if(curr==a[j].second){
