...
void solve() {
  ...
  ll a, b;
  cin >> a >> b;
  left = static_cast<long double>(1.0 / b);
  if (a == 0)
    right = 1e6;
  else
    right = static_cast<long double>(1.0 / a);
  bool ans = true;

  // Output to 8 decimal places
  ...
  for (int i = 2; i <= n; i++) {
    ...
    // nl;
    cin >> a >> b;
    long double cur_left = static_cast<long double>(i) / b;
    long double cur_right;
    if (a == 0)
      cur_right = 1e6;
    else
      cur_right = static_cast<long double>(i) / a;

    if (cur_right < left) ans = false;
    if (cur_left > right) ans = false;
...
//   nl;
  ...
  if (ans) {
    cout << left;  // Print the final answer
  } else {
    cout << -1;
  }
}
