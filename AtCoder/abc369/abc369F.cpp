...
vll bfs(ll x, ll y, ve<ve<pair<ll,ll>>> &rows, ve<ve<pair<ll,ll>>> &cols, const ll h, const ll w){
    vll ans;
    if(x==h && y==w){
        ans.push_back(n+1);
        return ans;
...
void solve(){
    ll h,w,n;
    cin>>h>>w>>n;
    ve<set<pair<ll,ll>> rows(h+1);
    ve<set<pair<ll,ll>> cols(w+1);
    map<pair<ll,ll>,ll> m;
    for(int i=1;i<=n;i++){
        ll x,y;
        cin>>x>>y;
        ...
        rows[x].insert({y,i});
        cols[y].insert({x,i});
    }rows[h].push_back({w,n+1});
    cols[w].push_back({h,n+1});

    vll ans = bfs(1,1,rows, cols, h, w);
}
