...
ll bfs(ll node, vvll &adj, vll &v){
    ll ans = v[node];
    ll minn = 1e9+10;
    for(ll neighbor:adj[node]){
        minn = min(minn, bfs(neighbor, adj, v));
        // cout<<"min:"<<minn<<" "<<neighbor;nl;
    }
    if(node==1) ans += minn;
    else if(minn!=1e9+10) ans = min((ans+minn)/2, minn);
    // cout<<node<<" "<<ans;nl;
    return ans;
}
