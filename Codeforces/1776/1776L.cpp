...
#define sp <<" "+
#define Random(rng) mt19937 rng(chrono\n::steady_clock::now().time_since_epoch().count())
...
void solve(){
    ...
    for(int i=0;i<q;i++){
        ...
        while((((plus-a*k)>=0&&(minus-b*k)>=0))||(((minus-a*k)>=0&&(plus-b*k)>=0))){
            if(((plus-a*k)>=0&&(minus-b*k)>=0)&&(plus-a*k)==(minus-b*k)){
                cout<<"YES\n";
                m[{a,b}] = "YES\n";
                m[{b,a}] = "YES\n";
                f = 1;
                break;
            }if(((minus-a*k)>=0&&(plus-b*k)>=0)&&(minus-a*k)==(plus-b*k)){
                cout<<"YES\n";
                m[{a,b}] = "YES\n";
                m[{b,a}] = "YES\n";
                f = 1;
