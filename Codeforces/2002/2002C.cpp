...
void solve(){
    ...
    cin>>v[n+1].fi>>v[n+1].se;
    ll dist = (v[0].fi-v[n+1].fi)*(v[0].fi-v[n+1].fi) + (v[0].se-v[n+1].se)*(v[0].se-v[n+1].se);

    bool flag = 1;
    for(int i=1;i<=n;i++){
        ll dist1 = (v[i].fi-v[n+1].fi)*(v[i].fi-v[n+1].fi) + (v[i].se-v[n+1].se)*(v[i].se-v[n+1].se);
        if(dist1<=dist){
            flag = 0;
            break;
        }
