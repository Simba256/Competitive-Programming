...
void solve(){
    ...
    ll cc = c;
    vll v(n),vcc(n);
    std::vector<pair<pair<ll,ll>,ll>> vv(n);
    rep(0,n,i){
        cin>>v[i];
        // cout<<"here1";
        vcc[i] = v[i];
        vv[i] = {{v[i]+min(ll(i+1),n-i),(min(ll(i+1),n-i)==(i+1))},ll(i)};
    }
    sort(all(vv),compare);

    ll ans = 0, left = 0, in = 0, mi = mll/1000;
    // cout<<
    cout<<"hereis the std::vector<int> v;:";
    rep(0,n,i){
        cout<<vv[i].first.first<<" "<<vv[i].first.second<<" "<<vv[i].second;nl;
    }nl;nl;
    ...
    rep(0,n,in){
        if(in<n && ((ans+vv[in].first.first+((left+vv[in].first.second)?0:min(vv[in].second+1-n+vv[in].second,2*mi+1-n)))<=c)){
            cout<<"here";
            ans += vv[in].first.first;
            left += vv[in].first.second;
            mi = min(mi,vv[in].second);
            if(left) break;
        }
    }
